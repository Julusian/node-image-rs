...
const runtimeRequire0 =
  typeof __webpack_require__ === "function"
    ...
    : createRequire(import.meta.url); // eslint-disable-line

// Use the modern dirname when available, fallback to __dirname for webpack compatibility
const dirname = import.meta.dirname || __dirname;

function runtimeRequire(name) {
  const candidates = [join(dirname, name), join(dirname, "prebuilds", name)];
...
function requireNative() {
  if (process.env.NAPI_RS_NATIVE_LIBRARY_PATH) {
    try {
      nativeBinding = runtimeRequire0(process.env.NAPI_RS_NATIVE_LIBRARY_PATH);
    } catch (err) {
      loadErrors.push(err);
    }
